...
bool multiStreamTrt::build(){
    ...
    parser->parse("example/data/mnist.prototxt" ,"example/data/mnist.caffemodel", *network, nvinfer1::DataType::kFLOAT);
    assert(network->getNbLayers() > 0  && "Network was not parsed correctly");
    std::cout<<"parsed succes"<<std::endl;
    ...
    bool splitted = splitNetwork(network ,splittedNetworks, builder,config);
...
bool multiStreamTrt::infer(std::string number){
    ...
    for (int j = 0; j < nNetworks; j++){
        ...
        inputSize[j] = std::accumulate(dims.d, dims.d + dims.nbDims, batchSize, std::multiplies<int>());
        // Create CUDA buffer for Tensor
        cudaMalloc(&inputBindings[j], inputSize[j] * sizeof(float));
        
        nvinfer1::Dims dims{mEngines[j]->getBindingDimensions(outputIndex)};
        size_t outputSize = std::accumulate(dims.d, dims.d + dims.nbDims, batchSize, std::multiplies<int>());
        // Create CUDA buffer for Tensor
    ...
    std::vector<std::string> dirs{"example/data/", "data/"};
    readPGMFile(locateFile(number + ".pgm", dirs), buffer, INPUT_H, INPUT_W);
    if (sizeof(buffer) != inputSize[0]){
        std::cout << "Couldn't read input Tensor" << std::endl; return false;
    }
    ...
    for (int i = 0; i < nNetworks; i++){
        ...
        if (i < nNetworks-1){
            if (outputs[i].size() != inputSize[i+1] || !hidden){
                std::cout<<"Can not read hidden input"<<std::endl;
                return false;
            }
            inputVec = outputs[i];
        }
...
bool multiStreamTrt::launchInference(std::vector<float> const& inputTensor, std::vector<float> & outputTensor, nvinfer1::ICudaEngine* engine, void* inputBinding, void* outputBinding){
...
std::vector<nvinfer1::INetworkDefinition*>& splittedNetworks, nvinfer1::IBuilder* builder,nvinfer1::IBuilderConfig* config){
...
/*
void multiStreamTrt::softmax(std::vector<float>& tensor)
{
